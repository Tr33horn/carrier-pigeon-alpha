...
/* -------------------------------------------------
   Sleep / flight realism helpers (same logic as before)
------------------------------------------------- */
...
function isSleepingAt(utcMs: number, offsetMin: number, cfg: SleepConfig) {
...
function awakeMsBetween(startUtcMs: number, endUtcMs: number, offsetMin: number, cfg: SleepConfig) {
...
function etaFromRequiredAwakeMs(sentUtcMs: number, requiredAwakeMs: number, offsetMin: number, cfg: SleepConfig) {
  let t = sentUtcMs;
...
function computeViewModel(l: RawLetter, cps: any[], realNowMs: number, direction: Direction) {
  ...
  const ignoresSleep = !!rule.ignoresSleep;
  const sleepCfg = rule.sleepCfg;
  ...
  // ✅ Step 3: speed + inefficiency are CODE rules, not DB.
  const speedKmh = Number(rule.speedKmh);
  ...
  const hasFlightInputs =
    ...
    speedKmh > 0 &&
    Number.isFinite(ineff) &&
    ineff > 0;

  // required “awake flight time”
  const requiredAwakeMs = hasFlightInputs ? (distanceKm / speedKmh) * ineff * 3600_000 : 0;
  ...
  if (sentMs != null && requiredAwakeMs > 0) {
    etaAdjustedMs = ignoresSleep
      ...
      : etaFromRequiredAwakeMs(sentMs, requiredAwakeMs, offsetMin, sleepCfg);
  ...
  const sleeping = canceled ? false : !delivered && !ignoresSleep ? isSleepingAt(calcNowMs, offsetMin, sleepCfg) : false;
  ...
  if (sentMs != null && calcNowMs > sentMs && requiredAwakeMs > 0) {
    traveledMs = ignoresSleep
      ...
      : awakeMsBetween(sentMs, Math.min(calcNowMs, etaAdjustedMs), offsetMin, sleepCfg);
  }
  ...
  return {
    ...
    // handy for debugging / future UI (optional fields; harmless)
    computed_speed_kmh: speedKmh,
    computed_inefficiency: ineff,
